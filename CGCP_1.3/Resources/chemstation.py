...
from Resources.cached_property import cached_property
from Resources.supported_versions import python2_and_3
...
class Injection(object):
    ...
    datetime_formats = (
        ...
        "\"%d-%b-%y  %H:%M:%S\"",  # "13-Jan-15 11:16:49"
        '\"%m/%d/%Y %I:%M:%S %p\" / \"\"', # "2/22/2019 2:19:06 PM" / ""
    )
    ...
    def _add_value_unit_to_metadata(self, name, value, unit):
        ...
        if unit.strip() != "":
            self.metadata[name] = str(value) + ' / ' + str(unit)
        else:
    ...
    def _parse_header(self):  # pylint: disable=too-many-branches
        ...
        for row in row_iter:
            name, value, unit = row
            # # add sample name and info
            if "name" in name:
                self.metadata['sample_name'] = value
            if "sample_info" in name:
                self.metadata['sample_info'] = value
        ...
        # Deal with columns -- section commented because not used and gives a Key error (for 'number_of_columns' and reason for that not clear)
